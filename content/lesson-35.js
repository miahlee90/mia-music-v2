/* Lesson 35 (5.3, formerly L36) — Minor Intervals (AEMT Book 2, Unit 9)
...
function MF_L35_shrink(container,fb){
  const ROUNDS=[
...
function MF_L35_convert(container,fb){
  const ROUNDS=[
  ...
  steps:[
    ...
    { say:"Hear the shrink with your own hands. \u{1F447} <b>For each pair: press the Major note, then the key one half step lower:</b>",
      try:{ type:"custom",
        ...
        mount:(container,fb)=>MF_L35_shrink(container,fb) } },
    ...
    { say:"Convert on the staff — the writer's skill. \u{1F447} <b>Pick the accidental that turns each Major interval into its minor twin:</b>",
      try:{ type:"custom",
        ...
        mount:(container,fb)=>MF_L35_convert(container,fb) } },
